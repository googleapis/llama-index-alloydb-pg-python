...
from .async_chat_store import AsyncAlloyDBChatStore
from .engine import AlloyDBEngine


class AlloyDBChatStore(BaseChatStore):
    """Chat Store Table stored in an AlloyDB for PostgreSQL database."""

    __create_key = object()

    def __init__(
        self, key: object, engine: AlloyDBEngine, chat_store: AsyncAlloyDBChatStore
    ):
        ...
        """
        if key != AlloyDBChatStore.__create_key:
            raise Exception(
    ...
    async def create(
        ...
        """
        coro = AsyncAlloyDBChatStore.create(engine, table_name, schema_name)
