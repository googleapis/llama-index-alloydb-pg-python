...
class TestAlloyDBChatStoreAsync:
    ...
    async def async_chat_store(self, async_engine):
        ...
        async_chat_store = await AlloyDBChatStore.create(
    ...
    async def test_init_with_constructor(self, async_engine):
        with pytest.raises(Exception):
            AlloyDBChatStore(engine=async_engine, table_name=default_table_name_async)
...
@pytest.mark.asyncio(loop_scope="class")
class TestAlloyDBChatStoreSync:
    @pytest.fixture(scope="module")
    def db_project(self) -> str:
        return get_env_var("PROJECT_ID", "project id for google cloud")
    ...
    async def sync_chat_store(self, sync_engine):
        ...
        sync_chat_store = AlloyDBChatStore.create_sync(
    ...
    async def test_init_with_constructor(self, sync_engine):
        with pytest.raises(Exception):
            AlloyDBChatStore(engine=sync_engine, table_name=default_table_name_sync)

    async def test_async_add_message(self, sync_engine, sync_chat_store):
        key = "test_add_key"
    ...
    async def test_aset_and_aget_messages(self, sync_chat_store):
        message_1 = ChatMessage(content="First message", role="user")
        message_2 = ChatMessage(content="Second message", role="user")
        messages = [message_1, message_2]
        key = "test_set_and_get_key"
    ...
    async def test_adelete_messages(self, sync_engine, sync_chat_store):
        messages = [ChatMessage(content="Message to delete", role="user")]
        key = "test_delete_key"
        sync_chat_store.set_messages(key, messages)
    ...
    async def test_adelete_message(self, sync_engine, sync_chat_store):
        message_1 = ChatMessage(content="Keep me", role="user")
        message_2 = ChatMessage(content="Delete me", role="user")
        messages = [message_1, message_2]
        key = "test_delete_message_key"
    ...
    async def test_adelete_last_message(self, sync_engine, sync_chat_store):
        message_1 = ChatMessage(content="Message 1", role="user")
        message_2 = ChatMessage(content="Message 2", role="user")
        message_3 = ChatMessage(content="Message 3", role="user")
        messages = [message_1, message_2, message_3]
    ...
    async def test_aget_keys(self, sync_engine, sync_chat_store):
        message_1 = [ChatMessage(content="First message", role="user")]
        message_2 = [ChatMessage(content="Second message", role="user")]
        key_1 = "key1"
        key_2 = "key2"
